...
export const NewScreening = () => {
  ...
  const assignedHealthCenter =
    user?.facilityName || "Polyclinique du Bon Berger";

  const [patientCodeInput, setPatientCodeInput] = useState("");
  const [loadedPatient, setLoadedPatient] = useState<PatientResponse | null>(
    null,
  );
  ...
  const openEditDialog = () => {
    ...
    setEditFormData({
      ...
      gender: loadedPatient.gender.toLowerCase(),
      guardianFirstName: loadedPatient.guardianFirstName,
      guardianLastName: loadedPatient.guardianLastName,
      guardianRelationship: loadedPatient.guardianRelationship,
      guardianPhone: loadedPatient.guardianPhone,
    });
    setEditErrors({});
  ...
  const validateEditForm = (): boolean => {
    ...
    else if (!/^[A-Za-z\s]+$/.test(editFormData.firstName))
      newErrors.firstName = "First name can only contain letters";

    if (!editFormData.lastName.trim())
      newErrors.lastName = "Last name is required";
    else if (!/^[A-Za-z\s]+$/.test(editFormData.lastName))
      newErrors.lastName = "Last name can only contain letters";

    if (!editFormData.birthDate) newErrors.birthDate = "Birth date is required";
    else {
      const bDate = new Date(editFormData.birthDate);
      const ageInYears =
        (new Date().getTime() - bDate.getTime()) / (1000 * 60 * 60 * 24 * 365);
      if (ageInYears >= 5)
        newErrors.birthDate = "Patient must be under 5 years old";
    }

    if (!editFormData.gender) newErrors.gender = "Gender is required";

    if (!editFormData.guardianFirstName.trim())
      newErrors.guardianFirstName = "Guardian first name is required";
    else if (!/^[A-Za-z\s]+$/.test(editFormData.guardianFirstName))
      newErrors.guardianFirstName =
        "Guardian first name can only contain letters";

    if (!editFormData.guardianLastName.trim())
      newErrors.guardianLastName = "Guardian last name is required";
    else if (!/^[A-Za-z\s]+$/.test(editFormData.guardianLastName))
      newErrors.guardianLastName =
        "Guardian last name can only contain letters";

    if (!editFormData.guardianRelationship)
    ...
    else if (!/^[0-9]{10}$/.test(editFormData.guardianPhone))
  ...
  const handleEditSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await patientsApi.update(loadedPatient.id, {
        firstName: editFormData.firstName,
        lastName: editFormData.lastName,
        birthDate: editFormData.birthDate,
        gender: editFormData.gender,
        guardianFirstName: editFormData.guardianFirstName,
        guardianLastName: editFormData.guardianLastName,
        guardianRelationship: editFormData.guardianRelationship,
        guardianPhone: editFormData.guardianPhone,
      });

      const resolvedAge = updated.birthDate
        ? computeAgeFromDate(updated.birthDate)
        : "Age unknown";
      setLoadedPatient({ ...updated, age: resolvedAge });
      setFormData((prev) => ({
        ...prev,
        childName: `${updated.firstName} ${updated.lastName}`,
        birthDate: updated.birthDate ?? "",
        gender: updated.gender.toLowerCase(),
        guardianName: `${updated.guardianFirstName} ${updated.guardianLastName}`,
  ...
  const handlePatientLookup = async () => {
    ...
    setIsLookingUp(true);
    try {
      const all = await patientsApi.getAll(user?.facilityId || undefined);
      const found = all.find(
        (p) =>
          p.patientCode.toLowerCase() === patientCodeInput.trim().toLowerCase(),
      );
      if (!found) {
  ...
  const calculateClassification = () => {
    ...
    if (muacNum < 11.5) {
      return {
        ...
        recommendation:
          "Severe Acute Malnutrition Detected. Submit service request to doctor immediately.",
      };
    } else if (muacNum >= 11.5 && muacNum < 12.5) {
      return {
        classification: "MAM" as const,
        recommendation:
          "Moderate malnutrition detected. Create nutrition order and consider service request to doctor for follow-up.",
      };
    } else {
      return {
        classification: "Normal" as const,
        recommendation:
          "Normal nutritional status. Create preventive nutrition order to support healthy growth.",
      };
    }
  };
    ...
    if (step === 2) {
      ...
      if (classification.classification === "SAM") {
        setServiceRequest({
          ...
          priority: "urgent",
          description:
            "Severe Acute Malnutrition detected during community screening",
          assignedDoctor: "",
        });
        ...
        setServiceRequest({
          ...
          priority: "routine",
          description:
            "Moderate Acute Malnutrition detected during community screening",
          assignedDoctor: "",
        });
      }
  ...
  const handleSaveScreening = async () => {
    ...
    if (nutritionOrder.enabled && !nutritionOrder.instructions.trim()) {
    ...
    try {
      ...
      if (serviceRequest.enabled) {
        try {
          const docId = serviceRequest.assignedDoctor
            ? Number(serviceRequest.assignedDoctor)
            : null;
          ...
          toast.error(
            `Screening saved, but failed to create service request: ${errorMsg}`,
          );
      ...
      if (nutritionOrder.enabled) {
        try {
          ...
          toast.success("Nutrition order generated successfully", {
            description: `Type: ${nutritionOrder.supplement}`,
          });
        } catch (noErr: any) {
          console.error(noErr);
          toast.error(
            `Screening saved, but failed to create nutrition order: ${noErr.message}`,
          );
        }
      }
    } catch (error: any) {
    ...
    <div className="p-6 space-y-6">
      ...
      <div>
        ...
        <p className="text-gray-600 mt-1">
          Record observations and create encounter for nutrition assessment
        </p>
      </div>
      ...
      <Card>
        <CardContent className="pt-6">
          <div className="flex items-center justify-between">
            <div
              className={`flex-1 text-center ${step >= 1 ? "text-green-600" : "text-gray-400"}`}
            >
              <div
            ...
            />
            <div
              className={`flex-1 text-center ${step >= 2 ? "text-green-600" : "text-gray-400"}`}
            >
              <div
                className={`w-8 h-8 rounded-full mx-auto mb-2 flex items-center justify-center ${
            ...
            />
            <div
              className={`flex-1 text-center ${step >= 3 ? "text-green-600" : "text-gray-400"}`}
            >
              <div
                className={`w-8 h-8 rounded-full mx-auto mb-2 flex items-center justify-center ${
      ...
      {step === 1 && (
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Enter patient details for the encounter
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            <div className="p-4 bg-gray-50 border border-gray-200 rounded-lg space-y-3">
              <p className="text-sm font-medium text-gray-900">
                Look up an existing patient by their Patient ID
              </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="birthDate">
                  {loadedPatient ? "Age" : "Date of Birth *"}
                </Label>
                {loadedPatient ? (
                  <Input
                    id="birthDate"
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) =>
                    !loadedPatient && setFormData({ ...formData, gender: v })
                  }
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2 md:col-span-2">
                <Label htmlFor="observationNotes">Observation Notes(optional)</Label>
                <Textarea
                  id="observationNotes"
                  ...
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      observationNotes: e.target.value,
                    })
                  }
                  rows={3}
                />
      ...
      {step === 3 && result && (
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              <CardTitle>
                Nutritional Classification (Auto-Calculated Read Only)
              </CardTitle>
              <CardDescription>
                Based on WHO standards for children under 5
              </CardDescription>
            </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      <Select
                        ...
                        onValueChange={(v) =>
                          setServiceRequest({ ...serviceRequest, priority: v })
                        }
                      >
                        <SelectTrigger>
                          <SelectValue />
                        ...
                        <SelectContent>
                          <SelectItem value="urgent">Urgent (SAM)</SelectItem>
                          <SelectItem value="routine">Routine (MAM)</SelectItem>
                        </SelectContent>
                      </Select>
                    </div>
                    <div className="space-y-2 md:col-span-2">
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription>
                      Create nutrition intervention to support healthy growth
              ...
              {nutritionOrder.enabled && (
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          ...
                          <SelectItem value="rusf">
                            RUSF (Ready-to-Use Supplementary Food)
                          </SelectItem>
                          <SelectItem value="micronutrients">
                            Micronutrient Powder
                          </SelectItem>
                          <SelectItem value="fortified-porridge">
                            Fortified Porridge
                          </SelectItem>
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <Label htmlFor="nutritionInstructions">
                        Instructions for Guardian *
                      </Label>
                      <Textarea
                        id="nutritionInstructions"
                        placeholder="Provide detailed feeding instructions, frequency, quantity, and next follow-up schedule..."
      ...
      <Dialog open={isEditDialogOpen} onOpenChange={setIsEditDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Update incorrect patient details. These changes will be saved to
              the database.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleEditSubmit} className="space-y-4 py-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <Label htmlFor="editGuardianFirstName">
                  Guardian First Name *
                </Label>
                <Input
              ...
              <div className="space-y-2">
                <Label htmlFor="editGuardianLastName">
                  Guardian Last Name *
                </Label>
                <Input
                  id="editGuardianLastName"
              ...
              <div className="space-y-2">
                <Label htmlFor="editGuardianRelationship">
                ...
                {editErrors.guardianRelationship && (
                  <p className="text-sm text-red-600 flex items-center gap-1">
                    <ErrorOutlineIcon className="h-3 w-3" />
                    {editErrors.guardianRelationship}
                  </p>
